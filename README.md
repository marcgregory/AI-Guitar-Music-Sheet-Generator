...
- For YouTube downloads that require sign-in or bot verification, configure browser cookies for the backend. On Render or similar hosted deploys, prefer `YOUTUBE_COOKIES_B64`: base64 encode the full raw Netscape-format `cookies.txt` contents and set that as the environment variable, then redeploy or restart the service. `YOUTUBE_COOKIES` still works, but hosted env editors can mangle multiline/tabbed cookie files. Leave `YOUTUBE_COOKIES_FILE` unset unless a real cookie file is mounted or included at that exact container path. If YouTube rejects valid cookies, set `YOUTUBE_PO_TOKEN` using yt-dlp's PO-token format, for example `web.gvs+TOKEN` or `mweb.gvs+TOKEN`, and optionally set `YOUTUBE_VISITOR_DATA`/`YOUTUBE_PLAYER_CLIENTS`. The committed placeholder cookie file is not real auth; direct audio upload remains the reliable fallback.
